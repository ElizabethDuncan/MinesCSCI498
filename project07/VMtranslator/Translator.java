	...
	public static void main(String[] fileName) throws IOException {
		...
		try {
			...
			while(textReader.hasNextLine()) {
				...
				//If a push command...
				if(type.length() == 4 && type.charAt(2) == 's'){
				...
				//if the type is pull....
					...
					String pullhack = (String)(popc.popget(segment));
		            ...
		            //Output results
		            System.out.println("//" + type + " " + segment + " " + index);
		            System.out.println( Pattern.compile( termToReplace ).matcher( pullhack ).replaceAll( replacedString ) );
		            //Output results to the output file
		            try {
		            	rFile.writeChars("//" + type + " " + segment + " " + index);
						rFile.writeChars(Pattern.compile( termToReplace ).matcher( pullhack ).replaceAll( replacedString ));
						rFile.writeChars("\n");
					}catch(IOException a) {
						a.printStackTrace();
					}
